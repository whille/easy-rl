...
class Config:
    ...
    def __init__(self):
        ...
        self.hidden_dim = 24  # 网络隐藏层
        ################################################################################

        ################################# 保存结果相关参数 ################################
        self.result_path = f"{curr_path}/outputs/{self.env_name}/results/"
        self.model_path = f"{curr_path}/outputs/{self.env_name}/models/"
...
def train(cfg, env, agent):
    ...
    for i_ep in range(cfg.train_eps):
        state = env.reset(seed=cfg.seed)[0]  # 重置环境，返回初始状态
        ep_reward = 0
        for _ in count():
