...
def create_moving_average_calculator(alpha=0.1):
    """
    创建一个用于计算滑动平均的闭包函数。

    参数:
    - alpha: 滑动平均的平滑系数，默认值为0.1。
    ...
    def calculate(current_reward):
        if ma_rewards:
            new_average = ma_rewards[-1] * (1 - alpha) + current_reward * alpha
        else:
            new_average = current_reward
        ma_rewards.append(new_average)
        return new_average
